	...
	try {
		...
		if (deleteCart) {
			...
			cartItems.forEach((product, index) => {
				...
				tableRows += `
                    <tr>
                        ...
                        <td style="padding: 8px; border: 1px solid #ddd;">KES ${price.toFixed(
													2
												)}</td>
                        <td style="padding: 8px; border: 1px solid #ddd;">${quantity}</td>
                        <td style="padding: 8px; border: 1px solid #ddd;">KES ${total.toFixed(
													2
												)}</td>
                    </tr>`;
			});
			...
			const message = `
                ...
                <p><strong>Grand Total:</strong> KES ${totalAmount.toFixed(
									2
								)}</p>
                <p>You can track your order <a href="${
									process.env.CLIENT_HOME_URL
								}/orders" target="_blank">here</a>.</p>
...
const getOrdersWithProductsController = async (req, res) => {
	try {
		const { orderId } = req.query;
		const response = await getOrderService(orderId);
		res.status(200).json(response);
